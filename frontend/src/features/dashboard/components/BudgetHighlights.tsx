  ...
  return (
    <section className="section-stack" aria-labelledby="budget-highlights-title">
      ...
      <div className="budget-grid">
        {cards.map((card) => {
          const targetBudget = card.overrideBudget ?? card.defaultBudget;
          ...
          return (
            <article className="budget-card" key={card.id}>
              <div className="budget-card__header">
                ...
                <div>
                  <p className="budget-card__eyebrow">
                    {card.overrideBudget ? 'Monthly override' : 'Default budget'}
                  </p>
                  <h4>{card.name}</h4>
                </div>
              </div>
              ...
              <p className="budget-card__caption">
                {formatCurrency(card.spent)} spent of {formatCurrency(targetBudget)}
              </p>
